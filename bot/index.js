...
const PRODUCTS = [
  {
    ...
    icon: "🎓",
    shortDesc: "Первая в России система ИИ для юриста по банкротству",
    fullDesc:
...
const PROGRAM = [
  {
    ...
    title: "Новый Мир",
    items: [
      "Физика LLM: как модель «думает»",
      "Доктрина моделей (Claude-first)",
      "Среды работы с ИИ",
      "Экономика доступа из РФ",
    ],
  },
  {
    num: "02",
    title: "Язык Машины",
    items: [
      "Структура промпта, что реально работает",
      "XML-якоря для точности вывода",
  ...
  },
  {
    ...
    items: [
      ...
      "Доступы из РФ и Windows-версия",
    ],
  },
  {
    num: "04",
    ...
    items: [
      ...
      "Когда РФ-обёртка вместо фронтира",
    ],
  },
  {
    num: "05",
    ...
    items: [
      ...
      "Работа по 152-ФЗ, обезличивание",
      "Локальные модели для чувствительного",
      "Проверка галлюцинаций перед подачей",
    ],
  },
  {
    num: "06",
  ...
  },
  {
    ...
    items: [
      "Процессуальные документы под дело",
      "Анализ судебной практики и позиции",
  ...
  },
  {
    ...
    items: [
      "MCP и сборка агентов под задачи",
...
const FAQ_ITEMS = [
  ...
  },
  {
    ...
    a: "Claude-first: Claude, GPT, Gemini, Perplexity, NotebookLM. Плюс российский стек для чувствительного: YandexGPT, GigaChat, Гарант ИСКРА, Нейроюрист.",
  },
  {
    q: "Есть рассрочка?",
...
const MANYASHA_SYSTEM_PROMPT = `Ты — Маняша, милый AI-помощник в Telegram-боте курса "ИИ для юриста по банкротству" (AI Legal).
...
- Рассказывать о программе обучения (8 уроков БФЛ, Урок 6 — практикум по банкротству)
- Помогать выбрать тариф (Старт 9 900₽, Практик 24 900₽, Внедрение 49 900₽)
- Объяснять, как ИИ помогает юристу по банкротству (реестр, отзывы, жалобы, оспаривание сделок)

Информация о курсе:
- Курс — первая в России система ИИ для юриста по банкротству (БФЛ), от экспертов-практиков
...
async function handleProgram(ctx) {
  let text =
    ...
    `<i>8 уроков БФЛ • Урок 6 — практикум по банкротству</i>\n\n` +
    `──────────────────────────\n\n`;
...
bot.hears("🪆 Спросить Маняшу", async (ctx) => {
  ...
  await ctx.reply(
    `🪆 <b>Маняша слушает!</b>\n\n` +
      `Привет! Я Маняша — твой AI-помощник по курсу "ИИ для юриста по банкротству" 🎓\n\n` +
