...
export class MascotRenderer {
    ...
    async setTexture(textureUrl: string, options: MascotTextureOptions = {}): Promise<boolean> {
        ...
        this.modelRoot.traverse((node: Object3D) => {
    ...
    setMorphTarget(targetName: string, influence = 1): boolean {
        ...
        this.modelRoot.traverse((node: Object3D) => {
            ...
            for (const [name, index] of Object.entries(dictionary)) {
                influences[Number(index)] = name === targetName ? safeInfluence : 0;
                if (name === targetName) {
                    applied = true;
                }
            }
    ...
    private detachAccessibilityListeners(): void {
        for (const entry of this.queryListeners) {
            if ('removeEventListener' in entry.query) {
                ...
                (entry.query as MediaQueryList & { removeListener?: (listener: MediaQueryChangeHandler) => void }).removeListener?.(entry.handler);
            }
        }
    ...
    private addMediaQueryListener(query: MediaQueryList, handler: MediaQueryChangeHandler): void {
        if ('addEventListener' in query) {
            ...
            (query as MediaQueryList & { addListener?: (listener: MediaQueryChangeHandler) => void }).addListener?.(handler);
        }
    ...
    private applyColorScheme(): void {
        ...
        if (this.ambientLight) {
            this.ambientLight.intensity = darkScheme ? 0.9 : 1.65;
        }
        this.keyLight?.color.set(darkScheme ? 0xa8c3ff : 0xffffff);
        if (this.keyLight) {
            this.keyLight.intensity = darkScheme ? 1.25 : 2.1;
        }
        this.fillLight?.color.set(darkScheme ? 0x6f8fc7 : 0xbdd7ff);
        if (this.fillLight) {
            this.fillLight.intensity = darkScheme ? 0.55 : 0.85;
        }
