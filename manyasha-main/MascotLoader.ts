...
export interface MascotLoaderWarning {
    code: 'LOD_POLYGONS_EXCEEDED' | 'MISSING_PROGRESSIVE_SKELETON' | 'KTX2_RENDERER_MISSING';
...
export class MascotLoader {
    ...
    private async loadInternal(
        ...
        const progressiveSkeletonPromise = request.skeleton
            ? this.loadAsset(request.skeleton, maxBytes, 'skeleton')
            : null;
        ...
        for (const [level, source] of lodEntries) {
            lodPromises.set(level, this.loadAsset(source, maxBytes, 'lod-' + level));
        }
    ...
    private async loadAsset(source: MascotAssetSource, maxBytes: number, stage: string): Promise<LoadedAsset> {
        ...
        this.validateManifest(manifest, source.url);
    ...
    private validateManifest(manifest: ParsedGltfManifest, url: string): void {
